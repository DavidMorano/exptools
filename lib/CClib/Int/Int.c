 ...
 *
 * Potential cast problems are marked with !!. Explicit casts
 * have been used to avoid warning messages.
 */
...
static const char * const _version = "version - 1 November 1992"; 
 ...
 */
void *INT_REP::operator new (size_t) {
...
/* INT_REP::delete:
 */
void INT_REP::operator delete (void* p)
{
	register INT_REP *ir = (INT_REP*)p;
...
INT_REP *INT_REP::Mult(INT_REP *q)
{
	...
	while (qptr < mq->wt) {
		...
		while (pptr < mp->wt) {
			...
			mt = cp*cq + carry + mcr;
			carry = (int)(mt>>16);			// mt/BASE. (!!ulong->int)
			mr->Alterc((int)(mt&0xFFFF), rptr++); 	// mt%BASE. (!!ulong->int)
		}
		offset++;
		if(carry != 0){
			mcr = (rptr == mr->wt ? 0 : *rptr);
			mr->Alterc((int)(mcr+carry), rptr);	// Why is mcr+carry < BASE? (!!long->int)
		}
	}
...
/* Print:
 * Print INT_REP.
 * We use the output mode supplied by the ostream.
 * After taking care of 0 and the minus sign,
 * we use the appropriate function to store the
 * representation in obuf.
 * Finally, we flush obuf onto the ostream.
 */
void INT_REP::Print(ostream &i)
{
...
{ 
	...
	length = (int)(Length() + ((shift + 15)>>4));	// (shift + 15)/16. (!!long->int)
	bitshift = (int)(shift & 0xF);					// shift%16. (!!long->int)
	newi = new INT_REP(length);
	newi->wt = newi->last;
...
INT_REP *INT_REP::RightShift(long shift)
{
	...
	offset = (int)(shift>>4);				// shift/16. (!!long->int)
...
const char * const version() { return _version; }
