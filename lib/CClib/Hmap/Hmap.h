...
#define Hmap(K,V)name4(Hmap_,K,_,V)
#define Hmapnode(K,V)name4(_Hnode_,K,_,V)
#define HmapnodeP(K,V)name4(_Hnodep_,K,_,V)
#define Hmappool(K,V)name4(_Hpool_,K,_,V)
#define Hmapiter(K,V)name4(Hmapiter_,K,_,V)
#define Hmapdefault(K,V)((const V&) name4(_HmapTDef_,K,_,V))


#define Hmapdeclare(S,T)						\
extern Pool Hmappool(S,T);						\
static T name4(_HmapTDef_,S,_,T);  /* Default value of type T. */	\
static S name4(_HmapSDef_,S,_,T);  /* Default value of type S. */	\
									\
...
class Hmap(S,T): public _Hmap_						\
  {									\
public:									\
    Hmap(S,T)(const T& def = name4(_HmapTDef_,S,_,T),			\
		...
		default_val(def) {}					\
    Hmap(S,T)(const Hmap(S,T)& m): _Hmap_(m),				\
			 default_val(m.default_val) { operator=(m); }	\
    ~Hmap(S,T)() { zap(); }						\
...
public:									\
    ...
    const S& key()							\
      {return p ? ((HmapnodeP(S,T)) p)->key : name4(_HmapSDef_,S,_,T);}	\
    T& value()								\
      {									\
...
#define Hmapimplement(S,T)						\
									\
Pool Hmappool(S,T)(sizeof(Hmapnode(S,T)));				\
static T name4(_HmapTJ_,S,_,T);						\
									\
void Hmap(S,T)::clear()							\
{									\
...
{									\
  name4(_HmapTJ_,S,_,T) = Hmapdefault(S,T);				\
  return name4(_HmapTJ_,S,_,T);						\
}
#endif
