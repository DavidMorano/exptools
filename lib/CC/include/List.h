...
/* class Lizt is the base class for all List(T)'s */
class Lizt {
protected:
    friend Liztiter;
    lnk* t;			// tail
...
#define OBJ__ATTLC(S,T) {List(T)::domain_error.raise(S);genericerror(0,S);}
...
/* Listdeclare(T) is the "macro" that the users will call when they want
   all of the declarations for the classes List(T) and Listiter(T). */
...
/* the following declarations of "Function object" classes are the
   ...
   included in the definition of class List(T). */
...
class List(T) : public Lizt {
private:
	void	make_select(Predicate(T)&, const List(T)&);
	void	make_select(Pred_obj(T)&, const List(T)&);
	void	make_reject(Predicate(T)&, const List(T)&);
	void	make_reject(Pred_obj(T)&, const List(T)&);
	void	make_map(Transformer(T)&, const List(T)&);
	void	make_map(Tran_obj(T)&, const List(T)&);
	List(T)(Predicate(T)& f, const List(T)& ps,
			List_select_dummy*) { make_select(f, ps); }
	List(T)(Pred_obj(T)& f, const List(T)& ps,
			List_select_dummy*) { make_select(f, ps); }
	List(T)(Predicate(T)& f, const List(T)& ps,
			List_reject_dummy*) { make_reject(f, ps); }
	List(T)(Pred_obj(T)& f, const List(T)& ps,
			List_reject_dummy*) { make_reject(f, ps); }
	List(T)(Transformer(T)& f, const List(T)& ps,
			List_map_dummy*) { make_map(f, ps); }
	List(T)(Tran_obj(T)& f, const List(T)& ps,
			List_map_dummy*) { make_map(f, ps); }
    	List(T)(const List(T)& a0, const List(T)& a1)
            : Lizt((Lizt&)a0, (Lizt&)a1) {}
    	List(T)(const List(T)& a0, const T& t);
protected:
...
public:
    ...
    static Objection domain_error;
    	...
    	List(T)(const List(T)&);
    	List(T)(const T& x);
    	List(T)(const T& x, const T& y);
    	List(T)(const T& x, const T& y, const T& z);
    	List(T)(const T& x, const T& y, const T& z, const T& w);
	List(T)(Gene_obj(T)&);
    	~List(T)() {}
    ...
    List(T)  operator+(const T& t) { return List(T)(*this, t); }
    List(T)& operator=(const List(T)& a) {
        return (List(T)&) (*(Lizt*)this = *(Lizt*)&a); }
    List(T)& put(const T& x) {
        return (List(T)&) Lizt::put(getnewlnnk_ATTLC(T)(x)); }
    List(T)& put(const List(T)& ll) {
        return (List(T)&) Lizt::put((Lizt&) ll); }
    T   unput();
    int unput(T& t) {
    ...
    List(T)&  unget(const T& x) { return (List(T)&)
                    Lizt::unget(getnewlnnk_ATTLC(T)(x)); }
    List(T)&  unget(const List(T)& ll) { return (List(T)&)
                    Lizt::unget((Lizt&) ll); }
    T   head() const;
    T   tail() const;
    ...
    int peek_next(T& t) const {
        return ((const Listiter(T)*)(&(((List(T)*)this)->myit)))->peek_next(t); }
    int peek_next(T*& t) const {
        return ((const Listiter(T)*)(&(((List(T)*)this)->myit)))->peek_next(t); }
    T   peek_next() const {
        return ((const Listiter(T)*)(&(((List(T)*)this)->myit)))->peek_next(); }
    int peek_prev(T& t) const {
        return ((const Listiter(T)*)(&(((List(T)*)this)->myit)))->peek_prev(t); }
    int	peek_prev(T*& t) const {
        return ((const Listiter(T)*)(&(((List(T)*)this)->myit)))->peek_prev(t); }
    T	peek_prev() const {
        return ((const Listiter(T)*)(&(((List(T)*)this)->myit)))->peek_prev(); }
    ...
    inline List(T)	select(Predicate(T)& p) const;
    inline List(T)	select(Pred_obj(T)& p) const;
    inline List(T)	reject(Predicate(T)& p) const;
    inline List(T)	reject(Pred_obj(T)& p) const;
    inline List(T)	map(Transformer(T)& f) const;
    inline List(T)	map(Tran_obj(T)& f) const;
};
inline List(T)
List(T)::select(Predicate(T)& p) const
{
	return List(T)(p, *this, (List_select_dummy*)0);
}
inline List(T)
List(T)::select(Pred_obj(T)& p) const
{
	return List(T)(p, *this, (List_select_dummy*)0);
}
inline List(T)
List(T)::reject(Predicate(T)& p) const
{
	return List(T)(p, *this, (List_reject_dummy*)0);
}
inline List(T)
List(T)::reject(Pred_obj(T)& p) const
{
	return List(T)(p, *this, (List_reject_dummy*)0);
}
inline List(T)
List(T)::map(Transformer(T)& f) const
{
	return List(T)(f, *this, (List_map_dummy*)0);
}
inline List(T)
List(T)::map(Tran_obj(T)& f) const
{
	return List(T)(f, *this, (List_map_dummy*)0);
}
PT_end
...
protected:
    T*	getAt(int i) { lnnk(T)* ll = ((lnnk(T)*)Liztiter::getAt(i));
                       if(ll) return &(ll->val); else return (T*)NULL;}
public:
    	Listiter(T)(List(T)& a) : Liztiter(*(Lizt*)&a) {}
...
/* List_generatordeclare(T) should be invoked by the user if there will
   be calls to the functions List(T)::generator() or List(T)::new_generator() */
PT_define List_generatordeclare(T)
class List_generator(T) : public Gene_obj(T) {
friend List(T);
	Listiter(T)	it;
		List_generator(T)(List(T)& p) : it(p) {}
public:
	T*	operator()();
};
List_generator(T)
...
class List_of_p(T) : public List(voidP) {
...
Objection List(T)::domain_error(L_error);
Pooldef_ATTLC(T)
...
List(T)::List(T)(const List(T)& a0, const T& t) : Lizt((Lizt&)a0)
{
    put(t);
}
List(T)::List(T)(const T& t)
{
    put(t);
}
List(T)::List(T)(const T& t, const T& u)
{
    put(t);
    put(u);
}
List(T)::List(T)(const T& t, const T& u, const T& v)
{
    put(t);
    put(u);
    put(v);
}
List(T)::List(T)(const T& t, const T& u, const T& v, const T& w)
{
    put(t);
    put(u);
    put(v);
...
List(T)::List(T)(Gene_obj(T)& gen)
{
    T*	pt;
    while (pt = gen())
	put(*pt);
}
List(T)::List(T)(const List(T)& a) : Lizt(*(Lizt*)&a) {}
T List(T)::unput() { lnnk(T)* ll = (lnnk(T)*)Lizt::unput();
    		 if(!ll) OBJ__ATTLC("unput: empty List",T)
                 T ans = ll->val; delete ll; return ans; }
T List(T)::get() { lnnk(T)* ll = (lnnk(T)*)Lizt::get();
...
void
List(T)::make_select(Predicate(T)& f, const List(T)& ps)
{
	Listiter(T)	it((List(T)&)ps);
	/* register */ T*	tp;
	while (it.next(tp))
		if (f(tp))
			put(*tp);
...
void
List(T)::make_select(Pred_obj(T)& f, const List(T)& ps)
{
	Listiter(T)	it((List(T)&)ps);
	/* register */ T*	tp;
	while (it.next(tp))
...
void
List(T)::make_reject(Predicate(T)& f, const List(T)& ps)
{
	Listiter(T)	it((List(T)&)ps);
	/* register */ T*	tp;
	while (it.next(tp))
...
void
List(T)::make_reject(Pred_obj(T)& f, const List(T)& ps)
{
	Listiter(T)	it((List(T)&)ps);
	/* register */ T*	tp;
	while (it.next(tp))
...
void
List(T)::make_map(Transformer(T)& f, const List(T)& ps)
{
	Listiter(T)	it((List(T)&)ps);
	/* register */ T*	tp;
	while (it.next(tp))
...
void
List(T)::make_map(Tran_obj(T)& f, const List(T)& ps)
{
	Listiter(T)	it((List(T)&)ps);
	/* register */ T*	tp;
	while (it.next(tp))
...
void
List(T)::do_each(Procedure(T)& f) const
{
	Listiter(T)	it(*(List(T)*)this);
	/* register */ T*	tp;
	while (it.next(tp))
		f(tp);
}
void
List(T)::do_each(Proc_obj(T)& f) const
{
	Listiter(T)	it(*(List(T)*)this);
	/* register */ T*	tp;
	while (it.next(tp))
...
T*
List(T)::do_while(Predicate(T)& f) const
{
	Listiter(T)	it(*(List(T)*)this);
	/* register */ T*	tp;
	while (it.next(tp))
...
T*
List(T)::do_while(Pred_obj(T)& f) const
{
	Listiter(T)	it(*(List(T)*)this);
	/* register */ T*	tp;
	while (it.next(tp))
...
T*
List(T)::do_until(Predicate(T)& f) const
{
	Listiter(T)	it(*(List(T)*)this);
	/* register */ T*	tp;
	while (it.next(tp))
...
T*
List(T)::do_until(Pred_obj(T)& f) const
{
	Listiter(T)	it(*(List(T)*)this);
	/* register */ T*	tp;
	while (it.next(tp))
...
void
List_of_p(T)::do_each(Procedure(T)& f) const
{
...
/* Listout(T) and List_of_pout(T) implement the iostreams output functions
   for List(T) objects and List_of_p(T) objects respectively. */
PT_define Listout(T)
ostream&
operator<<(ostream& oo, List(T)& ll)
{
    oo<<"( ";
    Listiter(T) l(ll);
