...
;; Modified by Bill Carpenter (billc@pegasus.att.com) 
;; for telnet use with cbnews
;; Tue Mar 24 16:08:57 EST 1992
...
(defvar tcptelnet-program-name "telnet"
...
specifying a port number to connect to."
  (let* ((telnetservice
		 ...
		 (cond
		  ...
		  (t service)))
		(telnetsuffix
		 ;; safety valve for domain addressing...
		 ;; all cbnews* get ".cb.att.com"
		 ;; anything else gets zilch
		 (cond
		  ((string-match "^cbnews[a-z]?$"  host) "cb.att.com")
		  ((string-match "^nntp[a-z]?$"  host) "cb.att.com")
		  )
		 )
		(proc (start-process
			   name buffer tcptelnet-program-name
			   (concat host telnetsuffix)
			   (if (integerp telnetservice)
				   (int-to-string telnetservice)
				 telnetservice)
			   )))
