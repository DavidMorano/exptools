  ...
  (let ((make-backup-files nil)
    ...
    (let ((files (append
		  ...
		  (sort (directory-files
			...
			'string<))))
      (while files
	(or (string-match "calc-maint\\|calc-macs" (car files))
...
Usage:  C-x C-f calc.texinfo RET
  ...
  (let ((srcbuf (current-buffer))
    ...
    (goto-char 1))
  (message "Wrote files calctut.tex and calcref.tex")
)
...
(defun calc-split-volume (number fix name other-name)
  ...
  (let ((pat (format "@c \\[fix-%s \\(.*\\)\\]\n" fix)))
	...
	(search-forward "}")
	(forward-char -1)
	(insert ", , " topic ", calc-info, the Calc " other-name))))
  (goto-char 1)
