...
(defun command-line-1 (command-line-args-left)
  ...
  (if (null command-line-args-left)
      (cond ((and (not inhibit-startup-message) (not noninteractive)
		 ...
		 (progn
		   (insert "
                      WELCOME TO (" emacs-distribution-type ") GNU EMACS 

     The advanced, self-documenting, customizable, extensible real-time
display editor, Emacs.  Copyright (C) 1993 Free Software Foundation, Inc.
...
To kill the Emacs job, type C-x C-c.       (`M-' means type ESC key first.)
Type C-h t for a tutorial on using Emacs.
Type C-h i to enter Info, which you can use to read GNU documentation.
Local tips are stored in the directory: " (getenv "GNUEMACS_LIB") "/local/tips.

GNU Emacs comes with ABSOLUTELY NO WARRANTY; type C-h C-w for full details.
It is provided as an experimental tool at AT&T Bell Laboratories. If you
have a question about GNU Emacs, or want a copy of the source code, you can
contact the " emacs-distribution-type " providers: 

              Charles_A_Webb@ATT.COM, Scott_F_Thompson@ATT.COM")
		     (insert (substitute-command-keys
       "Type \\[help-command] for help; \\[advertised-undo] to undo changes.  (`C-' means use CTRL key.)
To kill the Emacs job, type \\[save-buffers-kill-emacs].       (`M-' means type ESC key first.)
