...
;; Modified by Bill Carpenter (billc@pegasus.att.com) for DK use with cbnews
;; Mon Mar 23 12:55:35 EST 1992
...
specifying a port number to connect to."
  (let* ((dkservice
		 ...
		 (cond
		  ...
		  (t service)))
		(dkprefix
		 ;; safety valve for dialstrings...
		 ;; hosts cbnews and cbnewsa get "oh/cba/"
		 ;; all other cbnews* get "oh/cbh/"
		 ;; anything else gets zilch
		 (cond
		  ((string-equal "cbnews"   host) "oh/cba/")
		  ((string-equal "cbnewsa"  host) "oh/cba/")
		  ((string-match "^cbnews[a-z]?$"  host) "oh/cbh/")
		  ((string-match "^nntp[a-z]?$"  host) "oh/cbh/")
		  )
		 )
		(proc (start-process
			   name buffer tcpdk-program-name
			   (concat dkprefix host "." dkservice)
			   )))
