%%% `local guide' for exptools LaTeX.

%%% [For exptools tips directory [Dan Jacobson 8/1990]]: To make the
%%% PostScript (.ps) version of this file which you should have little
%%% trouble printing out, run latex on it twice, then run dvips, all
%%% as per the methods mentioned in the other files in this ``tips''
%%% directory.

\documentstyle[titlepage]{article}
%\date{27 October 1988\\              % Keep this date current
\date{6 August 1990\\              % Keep this date current
For \LaTeX\ Version 2.09}

% Modified by Dan_Jacobson@ATT.COM for AT&T Bell Laboratories exptools LaTeX
% 3...8/1990... [I should always remember to change \date]

%replaced ``Ultrix'' with ``UNIX'' --Dan
...
%for saying \LaTeX in the footnotes, because the ``a'' in \LaTeX is of
%a non standard font size:
%\newcommand{\LittleLaTeX}{LaTeX} %naw....
\newcommand{\LittleLaTeX}{\LaTeX}

\newcommand{\provider}{the current {\tt exptools} \TeX\ package providers}
%\newcommand{\contact}{Leslie Lamport}
\newcommand{\contact}{\provider}
...
%\author{Dan~Jacobson\\{}[from a template by Leslie~Lamport]}
...
%\footnote{ Do ask your local \TeX/\LittleLaTeX\
%guru/semi-guru first, like Mark~A.~Hartman (IL), Terry~L.~Anderson
%(NJ), Dan~Jacobson (IL) (a definite non-guru), etc.  (I have given
%their states in parentheses to ease your use of the {\tt post~-w}
%command to find their exact present locations.)  Also make use of the
%netnews newsgroups {\tt att.tex.text} and {\tt comp.text.tex}, etc.\
%for information sharing.  Netnews is a very good source of new ideas.
%---Dan}~\footnote{ Also note that \TeX,\LittleLaTeX, and {\tt dvips}
%are `optional' exptools installed upon demand for those exptool
%machine types for which there is a version compiled.  If you don't
%find it on your exptool machine, you might want to contact the
%official provider and request that it be installed.  ---Dan}
...
%In the early stages of deployment of
%\TeX\ and \LittleLaTeX\ at Bell Labs, less than the full-blown package
%has been installed.  Do bug the exptools \LittleLaTeX\ provider if you
%find something missing, though.  ---Dan
}
%
%replacing ``/usr/local/lib/tex82'' with ``~exptools/lib/tex/macros'' -Dan
%Carefully putting a \~ instead of ~ in the \tt environments.
%
%naaa, using $TOOLS instead -- 7/1990
...
\LaTeX.
%\footnote{
%May I suggest the exptool that I co-provide, the GNU Emacs editor
%({\tt gnuemacs}). ---Dan
%}
...
%But, e.g., on my machine I've got ``{\tt detex},'' however, so at
%least I'm happy.  By the way, I do all these operations (except
%walking to the printer) inside GNU Emacs' {\tt compile} mode using the
%UNIX {\tt make} command.  ---Dan
...
%\hbox{\verb|~exptools/lib/tex/macros|}.  You can change the directories in
...
%why the colons at front and back? --Dan 7/90:
\begin{verbatim}
...
There are several document styles and style options available\footnote{
%Again, when you read the word ``available'' remember that exptools
%\LittleLaTeX\ is in its early phases here.  ---Dan
...
%Not applicable, or available -Dan:
...
%not in Lamport's distribution, 1/1990 -Dan
%
...
%Ho ho ho, more Lamport localisms -Dan
%
...
%Dan: commented out... not included in distribution of 1/1990
%
...
%UNIX command {\tt man~aptex} to find out how to run this program.
...
%Dan: deleted because it's no big deal.
%
