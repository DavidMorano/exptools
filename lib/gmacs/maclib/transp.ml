; LastEditDate="Tue Jun  4 21:13:36 1991"
; TLH - added transpose-windows
...
(progn
    ...
    ;;;    This variable controls some magic inside the Transpose Line
    ...
    (defun
	  ...
	  ; the previous one if an argument is provided
	  
	  (transpose-windows dot1 dot2 buffer1 buffer2
	      (if prefix-argument-provided
		  (previous-window))
	      (setq dot1 (dot))
	      (setq buffer1 (current-buffer-name))
	      (next-window)
	      (setq dot2 (dot))
	      (setq buffer2 (current-buffer-name))
