...
/******************************************************************************
	...
	substitution on these variables from existing variables in the
...
extern char	*getenv3(const char *,int,const char **) ;
...
VECSTR		*vsp ;
{
	...
	for (i = 0 ; vecstr_get(vsp,i,&sp) >= 0 ; i += 1) {
	    ...
	    kp = sp ;
	    if ((tp = strchr(sp,'=')) == NULL) continue ;

	    vp = (tp + 1) ;
	    f = isprintlatin(kp[0] && 0xff) ;
	    if (f) {
		ch = (vp[0] & 0xff) ;
...
int	s1len, s2len ;
{
	...
	rs = varsub_expand(var1p,s2,slen,s1,s1len) ;
	slen = rs ;
